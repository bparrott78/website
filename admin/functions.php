...
function decrementComments($post_id){
	global $connection;
			...
			$decrease_comment_count_query="UPDATE `posts` SET `post_comment_count` = '{$comment_count}' WHERE `posts`.`post_id` = {$post_id}";
			$send_count=mysqli_query($connection,$decrease_comment_count_query);
			if(!$send_count){
				die("Query Failed").mysqli_error($connection);
			}
}
